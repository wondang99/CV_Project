...
def decoder(pred):
    ...
    # 하나의 Bounding Box 생성을 위해 min_index 생성
    ...
    if len(boxes) ==0:
        ...
        cls_indexs = [item.unsqueeze(0) for item in cls_indexs] #(n,)
    keep = nms(boxes,probs)
    return torch.index_select(torch.Tensor(boxes),0,keep).tolist(),torch.index_select(torch.Tensor(cls_indexs),0,keep).tolist(),torch.index_select(torch.Tensor(probs),0,keep).tolist()
...
if __name__ == '__main__':
    model = resnet50()
    print('load model...')
    model.load_state_dict(torch.load('best_100.pth'))
    model.eval()
    model.cuda()
    image_name = 'dog.jpg'
